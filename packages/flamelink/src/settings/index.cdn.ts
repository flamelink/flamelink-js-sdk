...
const settings: SetupModule = async (context: FlamelinkContext) => {
  if (context.dbType === 'rtdb') {
    const fn: any = await import('@flamelink/sdk-settings/dist/rtdb')
    return fn(context)
  }

  const fn: any = await import('@flamelink/sdk-settings/dist/cf')
  return fn(context)
}
