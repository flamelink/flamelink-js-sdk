...
const navigation: SetupModule = async (context: FlamelinkContext) => {
  if (context.dbType === 'rtdb') {
    const fn: any = await import('@flamelink/sdk-navigation/dist/rtdb')
    return fn(context)
  }

  const fn: any = await import('@flamelink/sdk-navigation/dist/cf')
  return fn(context)
}
